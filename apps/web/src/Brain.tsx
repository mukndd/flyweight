...
const COLORS=['#5f7482','#6f8796','#d29343'];
export function Brain({engine}:{engine:Engine}){
 ...
 useEffect(()=>{
  ...
  const draw=(time:number)=>{
   ...
   ctx.strokeStyle='#252b2e';ctx.lineWidth=.5;for(let x=10;x<w;x+=24)for(let y=10;y<h;y+=24){ctx.beginPath();ctx.moveTo(x-1,y);ctx.lineTo(x+1,y);ctx.stroke();}
   if(live&&graph&&graph.ids.length){
    ...
    for(const[a,b,weight]of graph.edges){const p=positions[a],q=positions[b],value=Math.abs(engine.activity[a]??0)+Math.abs(engine.activity[b]??0);ctx.strokeStyle=weight>=0?`rgba(210,147,67,${.07+value*.26})`:`rgba(111,135,150,${.07+value*.22})`;ctx.lineWidth=.6;ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(q.x,q.y);ctx.stroke();}
    for(let i=0;i<n;i++){const p=positions[i],v=Math.abs(engine.activity[i]??0),role=graph.roles[i],active=v>.035;if(active&&!reduced){ctx.fillStyle=`rgba(210,147,67,${.08+v*.18})`;ctx.beginPath();ctx.arc(p.x,p.y,3+v*8,0,Math.PI*2);ctx.fill();}ctx.globalAlpha=.34+Math.min(.66,v*4);ctx.fillStyle=active?'#d29343':COLORS[role];ctx.beginPath();ctx.arc(p.x,p.y,role?2.4:1.2+v*3,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;}
    ctx.font='10px Arial';ctx.fillStyle='#6f8796';ctx.textAlign='left';ctx.fillText('input',12,h-7);ctx.textAlign='right';ctx.fillStyle='#d29343';ctx.fillText('readout',w-12,h-7);
