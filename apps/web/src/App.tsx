...
import {CombatWorld3D} from './CombatWorld3D';
...
const palette=['ember','graphite','oxide','steel','cobalt','bone'];
...
function eventLabel(event:CombatEvent){
 const actor=event.actor===1?'Fly':'Human';
...
function ExperimentScene({engine,flyId,certified,context}:{engine:Engine;flyId:string;certified:number;context:'watch'|'fight'}) {
 ...
 return <section className="experiment-scene"><div className="scene-stage"><div className="scene-topline"><span>{flyId} vs Level {engine.publicLevel}</span><span>{engine.paused?'Paused':done?'Round complete':'Live simulation'}</span></div><CombatWorld3D engine={engine}/><div className="minimal-hud"><Health label={context==='fight'?'You':'Human'} value={engine.state.fighters[0].hp} side="opponent"/><div className="round-clock">{String(Math.max(0,Math.ceil((engine.state.limit-engine.state.frame)/60))).padStart(2,'0')}</div><Health label={flyId} value={engine.state.fighters[1].hp} side="fly"/></div><div className="stage-actions"><button onClick={()=>{engine.paused=!engine.paused;engine.matchStarted=true;}}>{engine.paused?'Resume':'Pause'}</button><button onClick={()=>engine.reset()}>Restart</button></div></div>
...
function EventStream({engine}:{engine:Engine}){
 const raw=(engine.state.events??[]).filter(event=>event.type!=='miss'||event.action!==5).slice(-16);
 const compressed:{time:string;label:string;count:number}[]=[];
 for(const event of raw){
  const label=eventLabel(event),time=clock(event.frame),last=compressed.at(-1);
  if(last&&last.label===label)last.count++;
  else compressed.push({time,label,count:1});
 }
 const rows=compressed.slice(-6);
 if(!rows.length)rows.push({time:clock(engine.state.frame),label:`selected ${ACTIONS[engine.state.fighters[1].action].toLowerCase()}`,count:1});
 return <section className="event-stream"><div className="small-heading">Match events</div>{rows.map((row,index)=><p key={`${row.time}-${index}`}><time>{row.time}</time><span>{row.count>1?`${row.count}x `:''}{row.label}</span></p>)}</section>;
}
...
function Inspector({engine}:{engine:Engine}){
 ...
 return <section className="sees-panel"><div><span>SEES</span><b>{sees.join(' · ')}</b></div><div><span>ACTION</span><b>{ACTIONS[engine.state.fighters[1].action].toLowerCase()}</b></div><div><span>RESULT</span><b>{latestResult(engine)}</b></div></section>;
}
...
function HumanChallenge({engine,flyId,certified,challenger,setChallenger,appearance,setAppearance,state,setState,startChallenge,openControls}:{engine:Engine;flyId:string;certified:number;challenger:string;setChallenger:(value:string)=>void;appearance:string;setAppearance:(value:string)=>void;state:'setup'|'fight'|'result';setState:(value:'setup'|'fight'|'result')=>void;startChallenge:()=>void;openControls:()=>void}){
 if(state==='setup')return <section className="challenge-setup"><div className="challenge-copy"><p className="kicker">HUMAN CHALLENGE</p><h1>Fight the Fly</h1><p>{flyId}. Certified Level {certified||'--'}.</p></div><div className="setup-preview"><Presentation3D engine={engine} variant="hero"/></div><div className="setup-form"><label>Name<input value={challenger} maxLength={24} onChange={e=>setChallenger(e.target.value)} placeholder="Optional"/></label><div><span>Appearance</span><div className="swatches">{palette.map(color=><button key={color} className={color} aria-pressed={appearance===color} onClick={()=>setAppearance(color)} title={color}><i/><b>{color}</b></button>)}</div></div><button className="primary-action" onClick={startChallenge}>START FIGHT</button></div></section>;
...
function ResultView({engine,flyId,challenger,startAgain}:{engine:Engine;flyId:string;challenger:string;startAgain:()=>void}){
 ...
 return <section className="result-view"><div className="result-scene" aria-hidden="true"><CombatWorld3D engine={engine}/></div><div className="result-card"><p className="kicker">MATCH RESULT</p><h1>{humanWon?'YOU WON':'THE FLY WON'}</h1><div className="result-versus"><span>{challenger.trim()||'Human'}</span><b>vs</b><span>{flyId}</span></div><div className="result-metrics"><span>{Math.round(engine.state.frame/60)} s</span><span>{Math.round(dealt)} dealt</span><span>{Math.round(taken)} taken</span><span>score {roundReward(engine.state,0).toFixed(1)}</span></div><button className="primary-action" onClick={startAgain}>FIGHT AGAIN</button><button className="secondary-action" onClick={()=>navigator.clipboard?.writeText(`Flyweight result: ${humanWon?'win':'loss'} vs ${flyId}, ${Math.round(dealt)} damage dealt.`)}>SHARE RESULT</button></div></section>;
}
...
function SciencePage({engine,flyId}:{engine:Engine;flyId:string}){
 ...
 return <section className="science-page"><div className="science-hero"><p className="kicker">SCIENCE</p><h1>What is actually running?</h1><p>The biological topology is fixed. Game sensors and motor readouts are artificial. Normal matches do not update weights.</p><button onClick={()=>void engine.fetchResearch()}>Refresh</button></div><PipelineDiagram/><div className="science-flow"><section><h2>{flyId}</h2><p>Public champion alias. Raw checkpoint identifiers stay here, not in the broadcast HUD.</p><dl><dt>Raw checkpoint</dt><dd>{engine.research?.current_champion?.checkpoint_id??engine.checkpoint}</dd><dt>Checkpoint hash</dt><dd>{engine.checkpointHash||'unavailable'}</dd><dt>Dataset hash</dt><dd>{engine.datasetHash}</dd></dl></section><section><h2>Controller Boundary</h2><p>FlyWire connectivity and initial strengths stay fixed. CEM trains only artificial sensory/readout adapters in isolated candidates.</p><p>Current browser matches are observations of a checkpoint, not online learning.</p></section><section><h2>Progress</h2><p>{cert?`Certified through Level ${cert.level}.`:'No level certification recorded yet.'}</p><div className="science-ladder">{Array.from({length:10},(_,i)=>i+1).map(level=><span key={level} className={cert&&level<=cert.level?'cleared':''}>{level}</span>)}</div><details><summary>Level manifest</summary>{ladder.map(level=><p key={level.version}>Level {level.index}: {level.difficulty}, {level.profile}, {level.min_matches} matches</p>)}</details></section><section><h2>Activity View</h2><p>No reliable anatomical coordinates are exposed to the web client. The public panel is a deterministic graph layout driven by live controller activations.</p><p>Mostly dim structure, bright active subset.</p></section></div><details className="deep-methods"><summary>Research history and controls</summary><div className="method-grid"><section><h3>Today</h3><p>{overview?.today?`${overview.today.matches_simulated} matches, ${overview.today.candidates_evaluated} candidates, ${overview.today.promotions} promotions.`:'Research registry unavailable.'}</p></section><section><h3>Topology Comparisons</h3>{comparison.length?comparison.slice(0,8).map(row=><p key={row.id}>{row.metrics.condition??row.suite}: {pct(row.metrics.win_rate)} win rate</p>):<p>First comparison batch running or unavailable.</p>}</section><section><h3>Human Results</h3>{engine.humanMatches?.matches?.length?engine.humanMatches.matches.slice(0,5).map(match=><p key={match.id}>{match.result.result.replace('_',' ')}: {Math.round(match.result.damage_dealt)} dealt</p>):<p>No challengers yet. Be the first.</p>}</section></div></details></section>;
}

function PipelineDiagram(){
 return <section className="pipeline-diagram" aria-label="Flyweight signal pipeline"><div><b>20</b><span>game signals</span></div><i/><div><b>encoder</b><span>artificial</span></div><i/><div className="connectome-step"><b>fixed FlyWire connectome</b><span>1,536 neurons</span></div><i/><div><b>readout</b><span>artificial</span></div><i/><div><b>14</b><span>actions</span></div></section>;
}
