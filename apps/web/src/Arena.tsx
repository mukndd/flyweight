...
export function Arena({engine}:{engine:Engine}){
 ...
 useEffect(()=>{
  class Ring extends Phaser.Scene{
   ...
   drawFighter(g:Phaser.GameObjects.Graphics,f:Fighter,color:number,floor:number,frame:number,neural:boolean){
    const x=f.x,y=floor-f.y,dir=f.face,walk=Math.sin(frame*.25)*Math.min(1,Math.abs(f.vx)/4),crouch=f.crouch?24:f.block?10:0,hit=f.stun?Math.sin(frame*2)*3:0;
    ...
    if(attacking&&extension>25){g.lineStyle(f.attack===6||f.attack===13?5:2,color,.3);g.strokeEllipse(fistX-dir*5,fistY,42,38);}
  ...
  const down=(e:KeyboardEvent)=>{if(document.querySelector('dialog[open]'))return;if(e.code==='Space'&&e.target instanceof HTMLButtonElement)return;if(e.target instanceof HTMLInputElement||e.target instanceof HTMLSelectElement||e.target instanceof HTMLTextAreaElement)return;if(['a','d','w','s','c','j','k','l',' '].includes(e.key.toLowerCase())){e.preventDefault();engine.keys.add(e.key.toLowerCase());}if(e.code==='Space'&&!e.repeat)engine.paused=!engine.paused;};
 ...
 return <div className="phaser-host" ref={host} aria-label="Live fighting arena" tabIndex={0}/>;
}
