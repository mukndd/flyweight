...
## Phase 2 authorization (2026-09-09)
The user explicitly authorized production/deployment preparation, Dockerfiles and CI, while preserving the current local architecture. These configuration files may support platform ports and production interfaces, but local verification still binds loopback only. Do not install Docker, launch public infrastructure, publish a repository, upload datasets, spend money or contact/configure remotes without explicit permission. Preserve v1 combat/replay source; expanded actions and adapters need new version identifiers. No original-source LICENSE until the user confirms a choice.

## Phase 3 (2026-09-09): Fly Brain online/trainable
User asked for one-command startup (real Fly Brain online by default, no silent synthetic fallback), a working CEM training pipeline (resume/curriculum/presets), and real evidence of learning while preserving the fixed-connectome/trainable-adapter boundary and CEM as the sole training method this phase (no PPO/SAC/DQN/gradient trainer without a documented recommendation first). Delivered: `npm run dev`/`dev:web`/`dev:brain`/`setup:brain`/`train:brain`/`eval:brain`; `trainer.py` `--preset {starter,serious}`/`--resume`/`--difficulties`; a promoted checkpoint (`candidate_78931b4895b24b48`, 100% held-out win rate) via the existing unmodified evaluation gate. See `docs/RESUME_BASELINE.md` for full results including an open question about seed-insensitive behaviour against medium/hard that needs follow-up. No remote, no deployment, no Docker build (not installed) touched this phase.
